  ...
  $.jCarousel.plugin("jcarouselSwipe", {
    ...
    _create: function() {
      ...
      this._instance._element.css(
        "touch-action",
        !this._instance.vertical ? "pan-y" : "pan-x"
      );
      this._carouselOffset =
        this.carousel().offset()[this._instance.lt] +
        parseInt(
          this.carousel().css(
            !this._instance.vertical ? "border-left-width" : "border-top-width"
          )
        ) +
        parseInt(
          this.carousel().css(
            !this._instance.vertical ? "padding-left" : "padding-top"
          )
        );
    ...
    _initGestures: function() {
      ...
      function dragMove(event) {
        ...
        if ((!self._instance.animating && xDiff > 10) || started) {
          ...
          if (!started) {
            ...
            lastItem = ($.isFunction(itemsOption)
              ? itemsOption.call(self._instance)
              : self._instance.list().find(itemsOption)
            ).last();
            edgeLT =
              self._instance.rtl && !self._instance.vertical
                ? self._instance.dimension(self._instance.list()) -
                  lastItem.position()[self._instance.lt] -
                  self._instance.clipping()
                : (lastItem.position()[self._instance.lt] +
                    self._instance.dimension(lastItem) -
                    self._instance.clipping()) *
                  -1;
          }
      ...
      function dragEnd(event) {
        ...
        if (
          started ||
          (!self._options.draggable && xDiff > 10 && xDiff > yDiff)
        ) {
          var newTarget = self._getNewTarget(
            startTouch[xKey] - currentTouch[xKey] > 0
          );
          newTarget =
            self._instance._options.wrap === "circular"
              ? newTarget.relative
              : newTarget.static;
    ...
    _getNewTarget: function(isSwipeToLT) {
      ...
      if (this._options.draggable) {
        while (true) {
          if (this._instance.rtl && !this._instance.vertical) {
            offsetDiff =
              target.offset()[this._instance.lt] +
              this._instance.dimension(target) -
              (this._carouselOffset + this._instance.clipping());
          } else {
            offsetDiff =
              target.offset()[this._instance.lt] - this._carouselOffset;
          }
      ...
      if (isToNext) {
        staticTarget =
          ...
          Math.abs(
            relativeTarget -
              this._options.perSwipe *
                Math.ceil(relativeTarget / this._options.perSwipe)
          );
      } else {
        staticTarget =
          staticTarget -
      ...
      if (this._instance._options.wrap === "first") {
        ...
        staticTarget = Math.max(
          0,
          Math.min(this._slidesCount - 1, staticTarget)
        );
      }
          ...
          if (curr.length === 0) {
            ...
            clonesBefore.push(
              items
                .eq(index)
                .clone()
                .attr("data-jcarousel-clone", true)
            );
          } else {
    ...
    _removeClones: function() {
      ...
      this._instance
        ...
        .remove();
      removeCLonesWidth =
        startPosition - this._instance.first().position()[this._instance.lt];
      if (removeCLonesWidth) {
        moveObj[this._instance.lt] =
          this._getListPosition() + removeCLonesWidth + "px";
