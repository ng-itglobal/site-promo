var $form = $('#sent-request-form');
        ...
        $form.on('submit', function (e) {
            ...
            $.ajax({
                ...
                .done(function (r, a, b) {
                    if (b && b.status === 200) {
                        $button.before('<p class="result-msg alert alert-info" role="alert">Заявка успешно отправлена, скоро мы с вами свяжемся.</p>');
                        $form[0].reset();
                    } else {
                        if (r.error) {
                            $button.before('<p class="result-msg alert alert-danger" role="alert">Произошла ошибка: "' + r.error +
                                '" — попробуйте связаться с нами через страницу \"Контакты\".</p>');
                        } else {
                            $button.before('<p class="result-msg alert alert-danger" role="alert">Произошла ошибка, попробуйте связаться с нами через страницу \"Контакты\".</p>');
                        }
                        console.log(r, a, b);
                    }
                })
                .fail(function (r, a, b) {
                    $button.before('<p class="result-msg alert alert-danger" role="alert">Произошла ошибка ' + r.status + ' (' + r.statusText + '), попробуйте связаться с нами через страницу \"Контакты\".</p>');
                    console.log(r, a, b);
                })
                .always(function () {
